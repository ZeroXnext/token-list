...
import {Chain, ChainMappingType, ListPath, SeenKey, TokenList} from '@tokenlist-builder/core';
import path from 'node:path';
import getPaths from './get-paths';

export default function load(outputDir: string, chainsMapping: ChainMappingType): [Map<ListPath, TokenList>, Set<SeenKey>] {
  ...
  return [new Map<ListPath, TokenList>(paths.map(key => {
    ...
    for (let {address, chainId} of list.tokens) {
      const {name, type} = chainsMapping?.get(chainId) as Chain;
